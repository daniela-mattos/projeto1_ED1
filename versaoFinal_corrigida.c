...
void pausa();
void consultaVetor(int vetor[TAM], int contaVetor);
void mostraVetor(int vetor[TAM], int contaVetor);
...
*/
    ...
    while(menu!=5) {
        ...
        printf("\n\t1 - Consulta posição"); //testar se a posição solicitada foi inserida pelo usuário
        printf("\n\t2 - Mostrar Vetor"); //remover TAM e mostrar até os ocupados
        printf("\n\t3 - Inserir");
        printf("\n\t4 - Remover");
        printf("\n\t5 - Sair\n");
        ...
        switch(menu) {
            case 1: 
                if(contaVetor!=0) consultaVetor(vetor, contaVetor);
                else printf("\n*\nNão há posições ocupadas no vetor.\n*\n*\n\n");
            break;
            case 2: mostraVetor(vetor, contaVetor);
            break;
            case 3: 
                if(contaVetor>=TAM) {
                    printf("\n*--=--*\nO vetor está cheio. Não é possível inserir mais códigos.\n");
                    mostraVetor(vetor, contaVetor);
                } else {
                    printf("\ncontaVetor: %i\n", contaVetor);
        ...
        if(numeroPosicao<0 || numeroPosicao>TAM-1) {
        	...
        	for(i=0;i<numeroPosicao;i++) {
                ...
                //verifica qual a primeira posição vazia até posição => insere o número
                    ...
                    printf("\n-------------------\n");
                    printf("Inserido %d na primeira posição disponivel: %d", numeroDigitado, i+1);
                    printf("\n-------------------\n");
        	...
        	for(i=TAM-1;i>(numeroPosicao);i--) {
                ...
                vetor[numeroPosicao] = numeroDigitado;
                (*contaVetor)++;
        	printf("Número inserido na posição %d. Vetor reordenado\n", numeroPosicao+1);
        	mostraVetor(vetor, *contaVetor);
        }
    } while(1);
}
